...
def gradient_penalty(D, x_real, x_fake):
    ...
    # 生成随机权重
    t = torch.rand(batch_size, 1, device=x_real.device)
...
for epoch in range(1000):
    for data_A,data_B in zip(dataloader_A,dataloader_B):
        real_A_,_ = data_A
        real_B_,_ = data_B
        real_A,real_B=real_A_.to(device),real_B_.to(device)
        ...
        loss_D_A=(loss_D_real_A+loss_D_fake_A)*0.5 + 10*gradient_penalty(net_D_A,real_A,net_G_BtoA(real_B))
        ...
        loss_D_B=(loss_D_real_B+loss_D_fake_B)*0.5 + 10*gradient_penalty(net_D_B,real_B,net_G_AtoB(real_A))
